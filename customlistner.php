...
switch($result)
{
	case "VERIFIED":
		...
		while($row = mysqli_fetch_array($rssubscr))
		{
		...
		{
			$id1 =  $id.'_5sdb';
			$squery = "update tbl_login set dbname = '".$id1."',status = '1',payment_status = '1',superadminid = '".$id."', parent = '".$id."' where uid = '".$id."'";
			...
			mysqli_query($sql,$chkentry) or die(mysqli_error($sql));
	
			$chkentry1 ="update tbl_plan set active_status = 1,subscr_id = '$sbcid'  where planid ='".$planid."'  ";
			mysqli_query($sql,$chkentry1) or die(mysqli_error($sql));

			if($id1!="")
			{
				$link =mysqli_connect("localhost","root","");
 				$sql = "CREATE DATABASE $id1 DEFAULT CHARACTER SET utf8  ";
				mysqli_query($link,$sql) or die('Could not connect: ' . mysqli_error($link));
				mysqli_select_db($link,$id1) or die ('Can\'t use foo : ');

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `adddepartment` (  `deptid` int(11) NOT NULL AUTO_INCREMENT,  `deptname` varchar(255) NOT NULL,
						  `depttimestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,  `uid` int(11) NOT NULL,  `superadminid` int(11) NOT NULL,  PRIMARY KEY (`deptid`))
						   ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=0 ");

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `addtemplate` (  `tempid` int(11) NOT NULL AUTO_INCREMENT,  `tempname` varchar(255) NOT NULL,
						  ...
						  `temptimestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,  `uid` int(11) NOT NULL,  `superadminid` int(11) NOT NULL,  PRIMARY KEY (`tempid`)) 	
						  ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=0 ");

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `tbl_project` (  `pid` int(11) NOT NULL AUTO_INCREMENT,  `tempid` int(11) NOT NULL,  `tempname` varchar(255) NOT NULL,
							...
							`timestamp` varchar(255) NOT NULL,  `status` varchar(255) NOT NULL DEFAULT '1',  `notes` varchar(255) NOT NULL, `auditedby` varchar(255) NOT NULL,
							`status_progress` int(11) NOT NULL DEFAULT '1',  `sync_status` int(11) NOT NULL,  `tstamp` int(11) NOT NULL,  PRIMARY KEY (`pid`)) 
							ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=0");

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `tbl_project_review` (  `prjid` int(11) NOT NULL AUTO_INCREMENT,  `pid` int(11) NOT NULL,  `qid` int(11) NOT NULL,
							`queid` int(11) NOT NULL,  `answer` int(11) NOT NULL,  `status` varchar(255) NOT NULL,  `uid` int(11) NOT NULL,  `timestamp` varchar(255) NOT NULL,
							`superadminid` int(11) NOT NULL,  PRIMARY KEY (`prjid`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=0");

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `tbl_questionanswer` (  `qaid` int(11) NOT NULL AUTO_INCREMENT,  `qid` int(11) NOT NULL,  `queid` int(11) NOT NULL,
							`checkitem` varchar(255) NOT NULL,  `description` longtext NOT NULL,  `uid` int(11) NOT NULL,  `status` int(11) NOT NULL,
				...
				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `tbl_questionnair` (  `qid` int(11) NOT NULL AUTO_INCREMENT,  `title` varchar(255) NOT NULL, `uid` int(11) NOT NULL,
							 `lid` int(11) NOT NULL,  `superadminid` int(11) NOT NULL,   `modified_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
							 `timestamp` varchar(255) NOT NULL,  PRIMARY KEY (`qid`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=0 ");

				mysqli_query($link,"CREATE TABLE IF NOT EXISTS `tbl_settings` (  `sid` int(11) NOT NULL AUTO_INCREMENT,  `key` varchar(255) NOT NULL,  `value` varchar(255) NOT NULL,
				...
				$sql_insert_queans="INSERT INTO `tbl_questionanswer`(`qaid`, `qid`,`checkitem`,`description`, `uid`, `timestamp`, `queid`	) 
VALUES
...
('', '7', 'Quadros de atividades ', 'Os quadros de atividades esto atualizados e so revistos
 regularmente?','$id',CURRENT_TIMESTAMP, '25'),
...
('', '8', 'Controles de exist
encias', 'Se han observado los controles de las existencias?','$id',CURRENT_TIMESTAMP, '23'),
...
$sql_insert_questionnair =" insert into  tbl_questionnair(`qid`,`title`,`uid`,`lid`,`superadminid`,`timestamp`,`status`)
							values('1','5s audit - english','$id','9','$id',CURRENT_TIMESTAMP,'1'),
							('2','5s audit - french','$id','4','$id',CURRENT_TIMESTAMP,'1'),
							('3','5s audit - italy','$id','7','$id',CURRENT_TIMESTAMP,'1'),
							('4','5s audit - swedish','$id','14','$id',CURRENT_TIMESTAMP,'1'),
							('5','5s audit - danish','$id','2','$id',CURRENT_TIMESTAMP,'1'),
							('6','5s audit - german','$id','5','$id',CURRENT_TIMESTAMP,'1'),
							('7','5s audit - portugal','$id','11','$id',CURRENT_TIMESTAMP,'1'),
							('8','5s audit - spanish','$id','13','$id',CURRENT_TIMESTAMP,'1'),
							('9','5s audit - russsian','$id','12','$id',CURRENT_TIMESTAMP,'1'),
							('10','5s audit - chinese','$id','1','$id',CURRENT_TIMESTAMP,'1'),
							('11','5s audit - greek','$id','6','$id',CURRENT_TIMESTAMP,'1'),
							('12','5s audit - japanese','$id','8','$id',CURRENT_TIMESTAMP,'1'),
...
$sql_insert_setting="INSERT INTO `tbl_settings` (`sid`,  `key`, `value`,`superadminid`,`uid`) 
VALUES('', 'default_language',  '9','$id','$id'), ('', 'default_language_question', '9', '$id', '$id')";
mysqli_query($link,$sql_insert_setting);
mysqli_close($link);
}
	$sqlemail = "select * from tbl_login where uid = $id";
	...
	$to = $email;
    $from = "Niftysol";
    $subject = "Welcome to Niftysol family";
	$message = "  <body bgcolor='#DCEEFC'>    <center> <b>Thank you for signing up for '5s'.Now you can login using following link:</b> <br>
    			<a href= 'http://5s.niftysol.com/$disname/$username/login'> Login here </a>    </center>  </body></html>";
   	$headers  = "From: Niftysol \r\n";
    ...
    include("dbconnect.php");   
	 ...
	 {
	 $sql_query = "select * from tbl_plan where subscr_id ='".$sbcid."'  ";
 	...
 	$utbl_plan ="update tbl_plan set active_status = 0, exp_date = '".$yesterdate."' where uid = ".$uid." and  subscr_id ='".$sbcid."' ";
	mysqli_query($sql,$utbl_plan) or die(mysqli_error($sql));
	$sqlq = "update tbl_login set status = 0 where superadminid =$uid  ";
