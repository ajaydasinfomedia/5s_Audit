...
include_once("function_manage_audit.php");
$suid=$_SESSION['said'];
$uid=$_SESSION['uid'];
...
<script type="text/javascript">   
    ...
    {    
     ...
     window.location.href=s+"/temp/"+Value1+"/dept/"+Value2+"/manage_audit";
    }                
</script>
<script type="text/javascript" language="javascript">
    ...
    function load(s)
    {                 
     window.location.href=s+"/manage_audit";          
    }            
    function load1(s1)
    {                       
     window.location.href=s1+"/manage_audit";          
    }
...
if(typeof(url) != "undefined" && url !== null &&  url.indexOf("page")!= -1 || url.indexOf("limit1")!= -1 )
{
	 	 document.getElementById("client").className = "activeTab";
		 document.getElementById("tab4").style.display="block"; 
		 document.getElementById("own").className = ""; 
...
if($_REQUEST['drptemplate'] != '' && $_REQUEST['drpdept'] != '')
{
	if($suid == $uid)
	{
		 $statement = "`tbl_project` where status=1 and tempid='".$_REQUEST['drptemplate']."' 
		 and deptid ='".$_REQUEST['drpdept']."' and `superadminid`=$suid  ORDER BY pdate DESC";
	}
	else
	{
		 $statement = "`tbl_project` where status=1 and tempid='".$_REQUEST['drptemplate']."' 
		 and deptid ='".$_REQUEST['drpdept']."' and `superadminid`=$suid and `uid` = $uid  ORDER BY pdate DESC";
	}	 
...
if($_REQUEST['drptemplate'] == '0' && $_REQUEST['drpdept'] != '' )
{
	if($suid == $uid)
	{
		$statement = "`tbl_project` where status=1 and deptid ='".$_REQUEST['drpdept']."' 
...
{
	if($suid == $uid)
	{ $statement = "`tbl_project` where status=1 and `superadminid`=$suid ORDER BY pdate DESC"; }
	else{ $statement = "`tbl_project` where status=1 and `superadminid`=$suid and `uid` = '$uid' ORDER BY pdate DESC"; }
		// $sql_query="select * from {$statement} LIMIT {$startpoint} , {$limit} ";
		$sql_query="select * from {$statement}";
}
}
else
{
	...
	{ $statement = "`tbl_project` where status=1 and `superadminid`=$suid ORDER BY pdate DESC";  }
	 ...
	 { $statement = "`tbl_project` where status=1 and `superadminid`=$suid and `uid`= '$uid'  ORDER BY pdate DESC";  }
    // $sql_query="select * from {$statement}  LIMIT {$startpoint} , {$limit}";
    $sql_query="select * from {$statement}";
}               
...
{    
  ...
  if($dept != '0' && $temp != '0'){
	if($suid == $uid)
	{ 
      $statement = "`tbl_project` where status=1 and tempid=$temp and deptid=$dept
		and `superadminid`=$suid ORDER BY pdate DESC ";
    }
	else
	{
...
if($dept!='0' && $temp=='0'){
	if($suid == $uid)
	{
    $statement = "`tbl_project` where status=1  and deptid =$dept and `superadminid`=$suid ORDER BY pdate DESC ";
	}
	else
	{
    $statement = "`tbl_project` where status=1  and deptid =$dept and `superadminid`=$suid and `uid`='$uid' ORDER BY pdate DESC ";
	}
    // $sql_query="select * from {$statement} LIMIT {$startpoint} , {$limit} ";
    $sql_query="select * from {$statement}";
}
if($dept=='0' && $temp!='0'){
	if($suid == $uid)
	{
    $statement = "`tbl_project` where status=1  and tempid=$temp and `superadminid`=$suid ORDER BY pdate DESC";
	}
	else
	{
	$statement = "`tbl_project` where status=1  and tempid=$temp and `superadminid`=$suid and `uid` = '$uid'
	 ORDER BY pdate DESC";
	}
     // $sql_query="select * from {$statement}  LIMIT {$startpoint} , {$limit} ";
     $sql_query="select * from {$statement}";
...
<div class="col-12 col-sm-9 col-md-9 col-lg-9 col-xl-9 float-right mt-1 mt-sm-4 dashboar px-0 px-sm-3">
		...
		<h6> <?php echo MANAGEAUDIT ?></h6></div>	
<?php 
if($manage_audit_right == 1){
?>		
<?php
if(strpos($_SERVER['REQUEST_URI'],'created'))
{
?>	
	<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 nNote nSuccess hideit">
	<p>
		<strong><?php echo SUCCESS.":" ?></strong><?php echo AUDITCREATEDWITHSCORE." !! " ?>
	</p>
	</div>
<?php
}
...
?>	
	<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 nNote nSuccess hideit">
	<p>
		<strong><?php echo SUCCESS.":" ?></strong><?php echo AUDITUPDATEDWITHSCORE." !! " ?>
	</p>
	</div>	
<?php
}
if(strpos($_SERVER['REQUEST_URI'],'create_update'))
{
?>	
	<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 nNote nSuccess hideit">
	<p>
		<strong><?php echo SUCCESS.":" ?></strong><?php echo AUDITUPDATED." !! " ?>
	</p>
	</div>
<?php
}
?>	<div class="container mt-3">
	<form action=<?php echo $_SESSION['dname']."/".$_SESSION['uname']."/manage_audit"?> class="mainForm" name="frmaudit" method="post">
		<div class="form-group row noborder">
			...
			<div class="col-12 col-sm-4 col-md-4 col-lg-4 col-xl-4 float-left">
				...
				<select name="drptemplate" style="color:#494949;"  id="t1" class="form-control form-control-sm imgopocity" >
					<option  value="0"  style="color:#494949;" selected="selected"><?php echo ALL ?></option>
						<?php
						...
						?>
						<option <?php if(isset($_REQUEST['cmdgo'])){ if( $_REQUEST["drptemplate"] == $tid  )
								...
								?>
									value="<?php echo $tid; ?>" style="color:#494949;">
			...
			<div class="col-12 col-sm-4 col-md-4 col-lg-4 col-xl-4 float-left mt-sm-0 mt-2">
				...
				?>
				<select name="drpdept" style="color:#494949;"  id="d1" class="form-control form-control-sm imgopocity">
						<option value="0" style="color:#494949;" selected="selected"><?php echo ALL ?></option>
					<?php
					while($row=mysqli_fetch_array($result))
					{
					...
					?>
					<option <?php if(isset($_REQUEST['cmdgo'])){ $dp=$_REQUEST["drpdept"];
					   if(isset($dp) && $dp == $did ){ echo 'selected="selected"';}}
					   else 
						{                                                                                                
						  if(strpos($_SERVER['REQUEST_URI'],'limit') && strpos($_SERVER['REQUEST_URI'],'page'))
						   {    
					    ...
					    ?>
									  value="<?php echo $did; ?>" style="color:#494949;"  ><?php  echo $departmentname;?>
					</option>
					<?php 
					}
					?>
			...
			<div class="col-12 col-sm-1 col-md-1 col-lg-1 col-xl-1 float-left mt-sm-0 mt-2">
				<input class="btn btn-success btn-sm" type="submit" name="cmdgo" onchange="load(this.value)" value="<?php echo GO_B ?>"/>
			</div>
		</div>
		...
		<div id="" class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 px-0">
			<table class="table table-striped table-bordered table-hover" id="dataTables-example" width="100%">
				...
				<tbody>
					...
					?>
					<tr>
						<td align="center" id="txttname" style="color:#494949;"><?php echo $row['tempname']; ?></td>
						<td align="center" id="txtdname" style="color:#494949;"><?php echo $row['deptname']; ?></td>
						<td align="center" id="txtdate" style="color:#494949;"><?php echo $sp; ?></td>
						<td align="center"id="txtdate" style="color:#494949;"><?php echo $row['pdate']; ?></td>
						<td align="center">
						<a href=<?php echo $_SESSION['dname']."/".$_SESSION['uname']."/".$row['pid']."/edit_audit"; ?> data-toggle="tooltip" title=" Edit " ><img class='mr10 p-2' alt='' src='images/icons/dark/pencil.png' /></a>
						...
						<img class='mr10 p-2' alt='' src='images/icons/dark/archive.png' /></a>
						<?php } ?>			 
						</td>
					</tr>
					<?php
